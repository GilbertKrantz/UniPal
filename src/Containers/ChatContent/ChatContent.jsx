...
const ChatContent = () => {
  ...
  useEffect(() => {
    const messageButton = document.getElementsByClassName(
      'ChatContent__message-content'
    )[chats.length - 1]
    ...
    if (sender != 'up') {
      if (userProfile.profilePicture == 'default') {
        return (
          <div
            className={
              header
                ? 'ChatContent__header-profile--default'
                : 'ChatContent__profile-image--default'
            }
          >
            <FaUser />
          </div>
        ...
        auth.onAuthStateChanged(async (user) => {
          ...
          return (
            <img src={url} alt='' className='ChatContent__profile-image' />
          )
        })
      }
      ...
      return (
        <img
          ...
          alt=''
          className='ChatContent__profile-image'
        />
      )
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        'https://unipal-be-api-production-8dd6.up.railway.app/api/chat',
        {
          method: 'POST',
  ...
  const startSpeechHelper = () => {
    document.getElementsByClassName('ChatContent__message-content')[
      chats.length - 1
    ].style.backgroundColor = '#402DD8'
  }
  ...
  const getSpeech = async (text) => {
    try {
      ...
      const response = await fetch(
        'https://unipal-be-api-production-8dd6.up.railway.app/api/elgenerate',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const startRecording = () => {
    setMessage('')
    setIsRecording(true)
    document.getElementsByClassName('ChatContent__input-bar')[0].disabled =
      true
        ...
        mediaRecorder.onstop = async () => {
          ...
          try {
            const response = await axios.post(
              'https://unipal-be-api-production-8dd6.up.railway.app/api/transcribe',
              formData,
              {
  ...
  const stopRecording = () => {
    ...
    document.getElementsByName('message')[0].placeholder =
      'Ketik apa yang ingin kamu tanyakan...'
  }
  ...
  const getMessage = () => {
    return chats.length == 0 ? (
      <p className=''>Tanya Sekarang</p>
    ) : (
      <div className='ChatContent__chat-item'>
        {chats.map((chat) => (
          <div
            className={
              'ChatContent__message' + (chat.sender != 'up' ? ' own' : '')
            }
          >
            <div className='ChatContent__chat-profile'>
              ...
              <span className='ChatContent__name'>
                {chat.sender != 'up' ? userProfile.username : 'UniPal'}
              </span>
            </div>
            <div className='ChatContent__message-container'>
              <button
                className='ChatContent__message-content'
                onClick={chat.sender != 'up' ? null : handleGenerateSpeech}
              >
                <Markdown remarkPlugins={[remarkGfm]}>{chat.msg}</Markdown>
              </button>
            </div>
  ...
  return (
    ...
    <div className='ChatContent'>
      <CSSTransition
        in={showProfile}
        timeout={200}
        classNames='UserProfile__transition'
        unmountOnExit
      >
      ...
      <div
        ...
        onClick={showProfile ? handleShowProfile : null}
      >
        <div className='ChatContent__header'>
          ...
          <div
            className='ChatContent__header-profile'
        ...
        <div
          className={
            'ChatContent__chat ' +
            (chats.length == 0
              ? 'ChatContent__chat--empty'
              : 'ChatContent__chat--filled')
          }
        >
          {getMessage()}
          {audioUrl && (
        ...
        <div className='ChatContent__input'>
          <form
            ...
            className='ChatContent__input--form'
          >
            <input
              autoComplete='off'
            ...
            <div className='ChatContent__chat-btn-container'>
              ...
              <button
                ...
                className='ChatContent__record-btn'
              >
                <FaMicrophone />
              </button>
            </div>
